...
type Compressor interface {
	...
	Decode([]byte, []byte) ([]byte, error)
	MaxEncodedLen(int) int
}
...
func (a *Allocator) Alloc(b []byte) (handle int64, err error) {
	buflen := len(b)
	if a.Compress != nil {
		buflen = a.Compress.MaxEncodedLen(buflen)
	}
	pbuf := buffer.Get(buflen)
...
func (a *Allocator) unlink(_, atoms, p, n int64) (err error) {
	switch {
	case p == 0 && n == 0:
	...
	default:
		...
		case 1:
			...
			case tagNotCompressed:
				...
				return
			case tagCompressed:
				return a.Compress.Decode(buf, first[1:dlen+1])
			}
		default:
			pcc := buffer.Get(1)
			...
			case tagCompressed:
				...
				return a.Compress.Decode(buf, zbuf)
			}
		}
	case 0:
		return buf[:0], nil
...
func (a *Allocator) makeUsedBlock(dst []byte, b []byte) (w []byte, rqAtoms int, cc byte, err error) {
	...
	rqAtoms = n2atoms(n)
	if a.Compress != nil && n > 14 { // attempt compression
		if dst, err = a.Compress.Encode(dst, b); err != nil {
			return
		}
...
func (a *Allocator) verifyUsed(h, totalAtoms int64, tag byte, buf, ubuf []byte, log func(error) bool, fast bool) (compressed bool, dlen int, atoms, link int64, err error) {
	...
	if cc == tagCompressed {
		if ubuf, err = a.Compress.Decode(ubuf, buf[:dlen]); err != nil || len(ubuf) > maxRq {
